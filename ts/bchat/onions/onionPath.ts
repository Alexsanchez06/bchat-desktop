...
export async function getGuardNodeOrSelectNewOnes() {
  if (guardNodes.length === 0) {
    ...
    if (guardNodesFromDb.length === 0) {
      window?.log?.warn(
        'BchatSnodeAPI::getGuardNodeOrSelectNewOnes - no guard nodes in DB. Will be selecting new guards nodes...'
      );
    } else {
...
async function buildNewOnionPathsWorker() {
  return pRetry(
    async () => {
      window?.log?.info('BchatSnodeAPI::buildNewOnionPaths - building new onion paths...');
      ...
      window?.log?.info(
        `BchatSnodeAPI::buildNewOnionPaths, snodePool length: ${allNodes.length}`
      );
