...
export const SignInTab = (props:any) => {
  ...
  if(signInMode == SignInMode.UsingRecoveryPhrase && !seedInputScreen){
   return( 
   <div className="session-registration__content">
      <GoBackMainMenuButton assent={()=>{props.assent(true)}} seedScreen={()=>{setSeedInputScreen(false)}}/>
      <RestoreSeedInput nextScreen={()=>{
        try{
        console.log("onclick",recoveryPhrase);
        mn_decode(recoveryPhrase);
      ...
      setRecoveryPhraseError(!value ? window.i18n('recoveryPhraseEmpty') : undefined);
